...
library(bda)
mediation.test(data$p.leng_centrado,data$Libros,data$p.civ_c)
mediation.test(data$p.leng_centrado,data$Ocupacion,data$p.civ_c)
mediation.test(data$p.leng_centrado,data$Educacion,data$p.civ_c)
...
data <- data %>%
  group_by(idschool) %>%
  mutate(Libros_m = mean(Libros,na.rm=T))

data$Libros_c = as.numeric(data$Libros - data$Libros_m) 
...
data <- data %>%
  group_by(idschool) %>%
  mutate(Ocupacion_m = mean(Ocupacion,na.rm=T))

data$Ocupacion_c = as.numeric(data$Ocupacion - data$Ocupacion_m)


data <- data %>%
  group_by(idschool) %>%
  mutate(Educacion_m = mean(Educacion,na.rm=T))

data$Educacion_c = as.numeric(data$Educacion - data$Educacion_m)




libros_lenguaje_nse_c = lmer(p.leng_centrado ~ 1 +  Libros_c +  Ocupacion_c + Educacion_c  + (1 | idschool), data = data, REML=F) 
libros_civica_nse_c = lmer(p.civ_c ~ 1 +  Libros_c +  Ocupacion_c + Educacion_c  + (1 | idschool), data = data, REML=F) 
libros_civica_nse_leng_c = lmer(p.civ_c ~ 1 + Libros_c +  Ocupacion_c + Educacion_c + p.leng_centrado  + (1 | idschool), data = data, REML=F) 

libros_lenguaje_nse_m = lmer(m_p.leng ~ 1 +  Libros_m +  Ocupacion_m + Educacion_m  + (1 | idschool), data = data, REML=F) 
libros_civica_nse_m = lmer(m_p.civ ~ 1 +  Libros_m +  Ocupacion_m + Educacion_m  + (1 | idschool), data = data, REML=F) 
libros_civica_nse_leng_m = lmer(m_p.civ ~ 1 + Libros_m +  Ocupacion_m + Educacion_m + m_p.leng  + (1 | idschool), data = data, REML=F) 
...
#custom.coef.names = c("Intercepto", "Libros (C)", "Lenguaje (C)", "Libros (M)", "Lenguaje (M)")
```
...
resultados = mediate(libros_lenguaje_nse_c, libros_civica_nse_leng_c, treat=c('Libros_c'), mediator='p.leng_centrado', boot=F)
summary(resultados) 
...
resultados = mediate(libros_lenguaje_nse_c, libros_civica_nse_leng_c, treat=c('Ocupacion_c'), mediator='p.leng_centrado', boot=F)
summary(resultados) 
...
resultados = mediate(libros_lenguaje_nse_c, libros_civica_nse_leng_c, treat=c('Educacion_c'), mediator='p.leng_centrado', boot=F)
summary(resultados) 


#https://towardsdatascience.com/doing-and-reporting-your-first-mediation-analysis-in-r-2fe423b92171 
#knitreg(libros_civica_nse_leng_c)
...
options(scipen=999) # Desactiva la notación científica
library(bda)
mediation.test(data$p.leng_centrado,data$Libros_c,data$p.civ_c)
mediation.test(data$p.leng_centrado,data$Ocupacion_c,data$p.civ_c)
mediation.test(data$p.leng_centrado,data$Educacion,data$p.civ_c)
...
```{r results='asis'}
results_0 = lmer(p.civ ~ 1 +  (1 | idschool), data = data, REML=F)

results_leg = lmer(p.civ ~ 1  + p.leng +  (1 | idschool), data = data, REML=F)


results_teoprevia = lmer(p.civ ~ 1 + Libros +  Ocupacion + Educacion +  +  ip_padres  + m_nse + ip_estudiante  +  m_cult_part    + (1 | idschool), data = data, REML=F)


results_control = lmer(p.civ ~ 1 + Libros +  Ocupacion + Educacion +  +  ip_padres  + m_nse + ip_estudiante  +  m_cult_part  + p.leng  + (1 | idschool), data = data, REML=F)



results_5 = lmer(p.civ ~ 1 + Libros +  Ocupacion + Educacion + ip_padres + ip_estudiante  +  m_cult_part  + p.leng*m_nse + (1 + p.leng| idschool), data = data, REML=F) 

#texreg::knitreg(list(results_0,results_leg ,results_teoprevia,results_control))

texreg::knitreg(list(results_leg ,results_teoprevia,results_control),custom.coef.names = c("Intercepto", "Comprensión Lectora","Más de 100 libros en el hogar", "Estatus ocupacional de los padres", "Padres con educación universitaria o más", 
                              "Interes político de los padres", "Media NSE de la escuela (M.NSE)","Interes político del estudiante","Cultura participativa de la escuela"),caption = "Análisis Multinivel",
        custom.header = list("Modelos para explicar nivel de Conocimiento Civico" = 1:3),custom.model.names = c("Modelo 1","Modelo 2","Modelo 3"),custom.gof.rows= list("R2 N2" = c("0.598","0.764","0.871"),"R2 N1" = c("0.343","0.043","0.358")  ))

        (2917.17-1172.12) / 2917.17
        (2917.17-685.93) / 2917.17
        (2917.17-373.86) / 2917.17

        (5789.35-3803.01) / 5789.35
        (5789.35- 5536.91) / 5789.35
        (5789.35-3712.00) / 5789.35
